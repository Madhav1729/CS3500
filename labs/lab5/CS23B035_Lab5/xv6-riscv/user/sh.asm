    ...
    11c8:	00e6ea63          	bltu	a3,a4,11dc <free+0x50>
{
  ...
  if(bp + bp->s.size == p->s.ptr){
    ...
    11e2:	02059813          	slli	a6,a1,0x20
    11e6:	01c85713          	srli	a4,a6,0x1c
  ...
  if(p + p->s.size == bp){
    ...
    11f6:	02061593          	slli	a1,a2,0x20
    11fa:	01c5d713          	srli	a4,a1,0x1c
...
malloc(uint nbytes)
{
  ...
  if(p == (char*)-1)
    ...
    1282:	b7e1                	j	124a <malloc+0x36>
      if(p->s.size == nunits)
    1284:	02e48c63          	beq	s1,a4,12bc <malloc+0xa8>
        p->s.size -= nunits;
    1288:	4137073b          	subw	a4,a4,s3
    128c:	c798                	sw	a4,8(a5)
        p += p->s.size;
    128e:	02071693          	slli	a3,a4,0x20
    1292:	01c6d713          	srli	a4,a3,0x1c
    1296:	97ba                	add	a5,a5,a4
        p->s.size = nunits;
    1298:	0137a423          	sw	s3,8(a5)
      freep = prevp;
    129c:	00001717          	auipc	a4,0x1
    12a0:	d6a73a23          	sd	a0,-652(a4) # 2010 <freep>
      return (void*)(p + 1);
    12a4:	01078513          	addi	a0,a5,16
      if((p = morecore(nunits)) == 0)
        return 0;
  }
}
    12a8:	70e2                	ld	ra,56(sp)
    12aa:	7442                	ld	s0,48(sp)
    12ac:	74a2                	ld	s1,40(sp)
    12ae:	7902                	ld	s2,32(sp)
    12b0:	69e2                	ld	s3,24(sp)
    12b2:	6a42                	ld	s4,16(sp)
    12b4:	6aa2                	ld	s5,8(sp)
    12b6:	6b02                	ld	s6,0(sp)
    12b8:	6121                	addi	sp,sp,64
    12ba:	8082                	ret
        prevp->s.ptr = p->s.ptr;
    12bc:	6398                	ld	a4,0(a5)
    12be:	e118                	sd	a4,0(a0)
    12c0:	bff1                	j	129c <malloc+0x88>
  hp->s.size = nu;
    12c2:	01652423          	sw	s6,8(a0)
  free((void*)(hp + 1));
    12c6:	0541                	addi	a0,a0,16
    12c8:	00000097          	auipc	ra,0x0
    12cc:	ec4080e7          	jalr	-316(ra) # 118c <free>
  return freep;
    12d0:	00093503          	ld	a0,0(s2)
      if((p = morecore(nunits)) == 0)
    12d4:	d971                	beqz	a0,12a8 <malloc+0x94>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    12d6:	611c                	ld	a5,0(a0)
    if(p->s.size >= nunits){
    12d8:	4798                	lw	a4,8(a5)
    12da:	fa9775e3          	bgeu	a4,s1,1284 <malloc+0x70>
    if(p == freep)
    12de:	00093703          	ld	a4,0(s2)
    12e2:	853e                	mv	a0,a5
    12e4:	fef719e3          	bne	a4,a5,12d6 <malloc+0xc2>
  p = sbrk(nu * sizeof(Header));
    12e8:	8552                	mv	a0,s4
    12ea:	00000097          	auipc	ra,0x0
    12ee:	b7c080e7          	jalr	-1156(ra) # e66 <sbrk>
  if(p == (char*)-1)
    12f2:	fd5518e3          	bne	a0,s5,12c2 <malloc+0xae>
        return 0;
    12f6:	4501                	li	a0,0
    12f8:	bf45                	j	12a8 <malloc+0x94>
